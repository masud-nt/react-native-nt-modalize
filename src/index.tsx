...
const ModalizeBase = (
  ...
  const handleAnimateClose = (
    ...
    Animated.parallel([
      ...
      setLastSnap(lastSnapValue ?? 0);
      setIsVisible(toInitialAlwaysOpen);
    });
  };

  const handleModalizeContentLayout = ({
    nativeEvent: { layout },
  ...
  const renderElement = (
    ...
    if (typeof Element === 'function') {
      return Element(); // Safely call the function if it's callable
    }

    if (React.isValidElement(Element)) {
      return Element; // Return React elements directly
    }
      ...
      >
        <Animated.View
          ...
          onLayout={(e: LayoutChangeEvent): void =>
            handleComponentLayout(e, name, absolute)
          }
        >
          {tag}
        </Animated.View>
